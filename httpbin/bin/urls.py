...
urlpatterns = [
    ...
    url(
        r"^relative-redirect/(?P<times>[0-9]+)$",
        views.relative_redirect,
        name="relative-redirect",
    ),
    url(
        r"^absolute-redirect/(?P<times>[0-9]+)$",
        views.absolute_redirect,
    ...
    url(
        r"basic-auth/(?P<user>.*)/(?P<passwd>.*)$", views.basic_auth, name="basic-auth"
    ),
    url(
        r"hidden-basic-auth/(?P<user>.*)/(?P<passwd>.*)$",
        views.hidden_basic_auth,
        name="hidden-basic-auth",
    ),
    url(
        r"digest-auth/(?P<qop>.*)/(?P<user>.*)/(?P<passwd>.*)/(?P<algorithm>.*)$",
        views.digest_auth,
        name="digest-auth",
    ),
